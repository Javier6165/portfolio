...
La Home vigente recorre hero → Snapshot → Work → Product practice → AI-native workflow → About → References → Playground → Contact. La intro es la única secuencia obligatoria. Después hay navegación libre; `Follow Javier` activa voluntariamente una secuencia Spotlight de arriba abajo y `Stop following` la abandona. `Director` funciona como una utility AI local: mantiene un pequeño modelo de atención, ritmo y estado, alterna criterio propio con el foco probable del visitante y, tras una pausa, puede acercar el cursor, comentar o hacer una corrección pequeña. Si el visitante vuelve a hacer scroll, la intervención se cancela inmediatamente. El contenido profesional nunca depende de esa capa y cualquier fallo interno la desactiva sin afectar a la web.
